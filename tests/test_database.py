...
def test_database_operations():
    ...
    try:
        ...
        print(f"PASSED Database initialized: {db_path}")
        ...
        print(f"PASSED Found {len(aapl_backtests)} AAPL backtest(s)")
        ...
        print(f"PASSED Found {len(bb_backtests)} Bollinger Bands backtest(s)")
        ...
        print(f"PASSED Added to monitoring with ID: {monitor_id}")
        assert monitor_id > 0, "Monitor ID should be positive"

        print("\nTest 6: Get monitored stocks...")
        monitored = db.get_monitored_stocks()
        print(f"PASSED Retrieved {len(monitored)} monitored stock(s)")
        ...
        print("PASSED Signal logged successfully")

        print("\nTest 8: Get signals...")
        signals = db.get_signals(days=30)
        print(f"PASSED Retrieved {len(signals)} signal(s)")
        ...
        print("PASSED Last checked timestamp updated")
        ...
        print(f"PASSED Removed from monitoring, {len(active_monitored)} active stocks remaining")
        ...
        print("PASSED Saved 5 additional backtests")

        all_backtests = db.get_backtests(limit=100)
        print(f"PASSED Total backtests in database: {len(all_backtests)}")
